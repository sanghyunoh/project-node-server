    ...
    app.post("/registerproductlist", function(req, res) {
        ...
        var attr = req.body.attr;
            ...
            pool.getConnection(function(err, conn) {
                ...
                var sql1 = "SELECT COUNT(*) AS count from productinfo;";
                ...
                "option3price) VALUES ('" + productid + "', '" + productname + "', '" + price + "', '" + count + "', '" + option1 + "'," +
                "'" + option1price + "', '" + option2 + "', '" + option2price + "', '" + option3 + "', '" + option3price + "');";
                    ...
                    if (rows) {
                        if(rows[0] == null){
                            ...
                            if(rows[0].count < count){
                                ...
                                var sql = "INSERT INTO salesinfo (productID, productname, count, option1, option2, option3, price) VALUES ('" +
                                productid + "', '" + productname + "', '" + count +"', '" + option1 +"', '" + option2 + "', '" + option3 + "', '"  + totalPrice + "');";
                                conn.query(sql, function(err) {
                                    if(err){
                                        console.log("에러");
                                    }
